...
specify different topside or platform types throughout the array. 
...
A section of line that has multiple parallel sections, such as a bridle or double chain section bounded by triplates, is specified with the 'subsections' key. Each list within the subsections key describes the sections of line connected in series for that individual line within the parallel.
    ...
    taut_bridle_double_chain:  # mooring line configuration identifier
        ...
        sections:                 #in order from anchor to fairlead
          ...
          - type: rope                # ID of a mooring line section type
            length: 500               # [m] unstretched length of line section
...
```yaml
mooring_line_types: # CHANGE added wire_160mm, chain_175mm
    ...
    chain_175mm::
        d_nom:    0.175      # [m] nominal diameter
        d_vol:    0.315      # [m] volume-equivalent diameter
        m:        612.5      # [kg/m] mass per unit length (linear density)
        EA:      2621e6      # [N] quasi-static stiffness
        MBL:     22.5e6      # [N] minimum breaking load ***(fully corroded)***
        # cost:      1486      # [$/m] cost per unit length ***Need to ask Chris***
        material:  R4 studless chain  # [-] material composition descriptor

    wire_160mm: 
        d_nom:    0.160      # [m] nominal diameter
        d_vol:    0.188      # [m] volume-equivalent diameter
        m:        135.5      # [kg/m] mass per unit length (linear density)
        EA:       2486e6     # [N] quasi-static stiffness
        MBL:    26.16e6      # [N] minimum breaking load
        #cost:      194       # [$/m] cost per unit length
        cost:      5.39       # [$/kg] cost per unit mass ***(From file TFI-BDFW-CT-IEA Venterra Costing Tool)***
        material: sheathed wire rope  # [-] material composition descriptor
...
rJtube is an optional parameter; if provided, and a Jtube relative position is not provided, it defines the radial distance of the connection point for the cable to the platform. If a Jtube is defined in the cable definition, rJtube is not used.
