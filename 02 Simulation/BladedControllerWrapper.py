...
class BladedController(object):
    def __init__(self, info):
        info.CanResumeSimulation = False  # if you wish to use the wrapper in an OrcaFlex restart analysis please contact Orcina
        ...
        try:
            ...
            if self.inputFileName is None:
                ...
                self.inputFileName = os.path.join(
                    modelDirectory, turbine.tags.InputFile
                )
                self.accInfile = self.inputFileName.encode(encoding)
    ...
    def update(self, info):
        if info.SimulationTime > self.lastupdateTime:
            ...
            for bladeIndex in range(self.controlledBladeCount):
                ...
                if self.useActuator:
                    actuatorOutput = self.actuators[0].output(pitchCommand)
    ...
    def Calculate(self, info):
        ...
        if info.ModelObject.PitchControlMode == "Common":
            ...
            for bladeIndex in range(self.controller.controlledBladeCount):
                ...
                info.StructValue[bladeIndex].Velocity = self.controller.pitchDot[
                    bladeIndex
                ]
                info.StructValue[bladeIndex].Acceleration = self.controller.pitchDotDot[
                    bladeIndex
                ]
...
class BaseYawMomentController(object):
    def Initialise(self, info):
        info.UpdateDuringStatics = False
        self.turbineName = info.ModelObject.tags.get("TurbineName", None)
        if self.turbineName is None:
            raise Exception(
                "To use a constraint as a yaw controller, it must have a tag named TurbineName, the value of which is the associated turbine object name. No tag was found."
            )

        if not self.turbineName in (object.Name for object in info.Model.objects):
            raise Exception(
                "To use a constraint as a yaw controller, it must have a tag named TurbineName, the value of which is the associated turbine object name. The turbine name {} is unrecognised.".format(
                    self.turbineName
                )
            )
    ...
    def Calculate(self, info):
        if self.yawStiff is None:
            self.yawStiff = info.Workspace[self.turbineName + "YawStiff"]

        if self.yawDamp is None:
            self.yawDamp = info.Workspace[self.turbineName + "YawDamp"]

        if self.controller is None:
            self.controller = info.Workspace.get(
                info.Model[self.turbineName].handle.value, None
            )
        else:
            self.YawMoment = (
                self.yawStiff * self.controller.yaw
                + self.yawDamp * self.controller.yawDot
            )


class YawMomentControllerNacelleSide(BaseYawMomentController):
    def Initialise(self, info):
        super().Initialise(info)
        info.Workspace[self.turbineName + "YawStiff"] = (
            info.ModelObject.RotationalStiffness * 180 / numpy.pi
        )
        info.Workspace[self.turbineName + "YawDamp"] = (
            info.ModelObject.RotationalDamping * 180 / numpy.pi
        )

    def Calculate(self, info):
        super().Calculate(info)
        info.Value = self.YawMoment


class YawMomentControllerTowerSide(BaseYawMomentController):
    def Initialise(self, info):
        super().Initialise(info)
